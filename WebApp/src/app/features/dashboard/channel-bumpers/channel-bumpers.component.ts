...
export class ChannelBumpersComponent implements OnInit, AfterViewInit {
  ...
  onChannelChange(channelId: number) {
    ...
    this.erasService.getByChannel(channelId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
  ...
  loadBumpers(eraId: number) {
    this.bumpersService.getByEra(eraId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
  ...
  openForm(bumper?: ChannelBumperResponse) {
    ...
    if (!eraId) {
      this.showError('Select an era first');
      return;
    }
