...
export class ChannelErasComponent implements OnInit, AfterViewInit {
  ...
  openForm(era?: ChannelEraResponse) {
    ...
    if (!channelId) {
      this.showError('Select a channel first');
      return;
    }
    ...
    dialogRef.afterClosed().pipe(takeUntilDestroyed(this.destroyRef)).subscribe((result) => {
      ...
      if (era) {
        this.channelErasService
          .update(era.id, result.data as ChannelEraRequest)
          .pipe(takeUntilDestroyed(this.destroyRef))
          .subscribe({
          next: () => {
            this.loadEras(channelId);
        ...
        this.channelErasService
          .create(channelId, result.data as ChannelEraRequest)
          .pipe(takeUntilDestroyed(this.destroyRef))
          .subscribe({
          next: () => {
            this.loadEras(channelId);
  ...
  deleteEra(era: ChannelEraResponse) {
    this.channelErasService.delete(era.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        this.loadEras(this.selectedChannelId()!);
